...
function getTeamLookupMap(ss) {
    const teamLookupValues = ss.getSheetByName(TEAM_LOOKUP_SHEET).getDataRange().getValues();
    const teamLookupMap = {};
...
function getTeamResults(ss) {
    ...
    for (const resultSheetName of RESULT_SHEETS) {
        ...
        Object.entries(teamBallotResults).forEach(([teamNum, ballotResults]) => {
            ...
            teamResults[teamNum].ballotsWon += totalBallots * BALLOTS_PER_ROUND / ballotResults.length;
...
function getSchoolResults(teamResults, teamLookupMap) {
    ...
    const ballotSum = Object.values(schoolResults).reduce((acc, curr) => acc + curr.ballotsWon, 0);
    ...
    Object.entries(schoolResults).forEach(([schoolName, schoolResult]) => {
        ...
        schoolResult.overallScore = (schoolResult.ballotsWon / schoolResult.possibleBallots) * (schoolResult.ballotsWon / schoolResult.totalPossibleBallots);
    });
...
function writeResults(schoolResults) {
    ...
    const resultRows = Object.entries(schoolResults)
        ...
        .map(([schoolName, schoolResult]) =>
            [schoolName, schoolResult.ballotsWon, schoolResult.possibleBallots, schoolResult.totalPossibleBallots, schoolResult.overallScore]);
    resultSheet.getRange(2, 1, resultRows.length, resultRows[0].length).setValues(resultRows);
}
