...
const createIndividualResultsOutput = (competitorMap) => {
    ...
    competitorMap.forEach((competitorObject, competitorKey) => {
        ...
        const individualResult = [competitorInfo["team"], competitorInfo["side"], totalRankValue, competitorInfo["name"],   teamNameMap[competitorInfo["team"]],];
...
const tabulateIndividualBallot = (ballot, index, rankingType, firstRound, lastRound, competitorMap) => {
    ...
    competitorObject[roundNumber].push({rankValue: ballot[IndividualResultsIndices.RANK_VALUE], judgeName: ballot[IndividualResultsIndices.JUDGE_NAME]});
}
