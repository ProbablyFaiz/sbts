function CreateTeamBallotFolders() {
    ...
    Object.entries(teamInfoMap).forEach(([teamNumber, teamInfo]) => {
        if (teamInfo.ballotFolderLink?.length) {
            SheetLogger.log(`Folder for team ${teamNumber} already exists, skipping...`);
        }
        else {
            SheetLogger.log(`Creating ballot publish folder for team ${teamNumber}...`);
            const teamFolder = getChildFolder(context.exportFolder, `Team ${teamNumber}`)
    ...
    SheetLogger.log(`Created ${foldersCreated} team folders for ballot publishing. Run the email script to email these links to teams.`);
}
